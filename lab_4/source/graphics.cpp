...
Buffer::Buffer(size_t size, const void* data,
	...
	// Проверка на некорректный размер
	if (size == 0 || size > SIZE_MAX / 2) {
		throw std::runtime_error("Invalid buffer size for allocation");
	}
...
size_t Buffer::structureAlignment(size_t struct_size) {
	...
	// Если alignment не инициализирован, используем безопасное значение по умолчанию
	if (alignment == 0) {
		return struct_size;
	}

	return ((struct_size + alignment - 1) & ~(alignment - 1));
}
...
: width{width}, height{height}, format{format} {
	// Проверка на некорректные размеры текстуры
